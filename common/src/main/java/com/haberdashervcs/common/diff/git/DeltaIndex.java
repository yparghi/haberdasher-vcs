...
/*
 * Notes for Haberdasher:
 *
 * These files were copied and pasted out of JGit, then adapted for our streamy purposes. The changes:
 * - byte[] -> RandomAccessBytes
 * - long (for sizes and offsets) is replaced with int. If we assume a file size limit of <= 2GB, this isn't a problem?
 */
...
class DeltaIndex {
     ...
     */
    private final RandomAccessBytes src;
     ...
     * {@link #entries} that the consecutive list of blocks with that
     ...
     */
    public DeltaIndex(RandomAccessBytes sourceBuffer) {
        src = sourceBuffer;
     ...
     *            within this index.
     * @throws java.io.IOException the output stream refused to write the instructions.
     */
    public void encode(OutputStream out, RandomAccessBytes res) throws IOException {
     ...
     * @throws java.io.IOException the output stream refused to write the instructions.
     */
    public boolean encode(OutputStream out, RandomAccessBytes res, int deltaSizeLimit)
            throws IOException {
        ...
        if (end < BLKSZ || table.length == 0)
            return enc.insert(res, 0, res.length());
        ...
        while (blkEnd < end) {
            ...
            if (entryIdx == 0) {
                ...
                //
                hash = step(hash, res.at(blkPtr), res.at(blkEnd));
                ++blkPtr;
                ++blkEnd;
                continue;
            }
    ...
    private static int fwdmatch(RandomAccessBytes res, int resPtr, RandomAccessBytes src, int srcPtr) {
        int start = resPtr;
        for (; resPtr < res.length() && srcPtr < src.length(); resPtr++, srcPtr++) {
            if (res.at(resPtr) != src.at(srcPtr))
                break;
        }
        return resPtr - start;
    }

    private static int negmatch(RandomAccessBytes res, int resPtr, RandomAccessBytes src, int srcPtr,
                                int limit) {
        if (srcPtr == 0)
            return 0;
    ...
    static int hashBlock(RandomAccessBytes raw, int ptr) {
        int hash;
        ...
        //
        hash = ((raw.at(ptr) & 0xff) << 24) //
                | ((raw.at(ptr + 1) & 0xff) << 16) //
                | ((raw.at(ptr + 2) & 0xff) << 8) //
                | (raw.at(ptr + 3) & 0xff);
        hash ^= T[hash >>> 31];

        hash = ((hash << 8) | (raw.at(ptr + 4) & 0xff)) ^ T[hash >>> 23];
        hash = ((hash << 8) | (raw.at(ptr + 5) & 0xff)) ^ T[hash >>> 23];
        hash = ((hash << 8) | (raw.at(ptr + 6) & 0xff)) ^ T[hash >>> 23];
        hash = ((hash << 8) | (raw.at(ptr + 7) & 0xff)) ^ T[hash >>> 23];

        hash = ((hash << 8) | (raw.at(ptr + 8) & 0xff)) ^ T[hash >>> 23];
        hash = ((hash << 8) | (raw.at(ptr + 9) & 0xff)) ^ T[hash >>> 23];
        hash = ((hash << 8) | (raw.at(ptr + 10) & 0xff)) ^ T[hash >>> 23];
        hash = ((hash << 8) | (raw.at(ptr + 11) & 0xff)) ^ T[hash >>> 23];

        hash = ((hash << 8) | (raw.at(ptr + 12) & 0xff)) ^ T[hash >>> 23];
        hash = ((hash << 8) | (raw.at(ptr + 13) & 0xff)) ^ T[hash >>> 23];
        hash = ((hash << 8) | (raw.at(ptr + 14) & 0xff)) ^ T[hash >>> 23];
        hash = ((hash << 8) | (raw.at(ptr + 15) & 0xff)) ^ T[hash >>> 23];
